...
object CacheIntegrityChecker {
    ...
    /**
     * Sincroniza LoginData
     */
    fun syncLoginDataToDatabase() {
        val logs = mutableListOf<String>()
        logs.add("=".repeat(70))
        logs.add("SINCRONIZAÇÃO LOGINDATA - ${dateFormat.format(Date())}")
        logs.add("=".repeat(70))
        ...
        try {
            transaction(TotalEssentials.getCore().sql) {
                val cacheKeys = LoginData.password.getMap().keys.mapTo(HashSet()) { it.lowercase() }

                val dbRecords = LoginDataSQL.selectAll()
                    .associate { row ->
                        row[LoginDataSQL.player].lowercase() to LoginDbRecord(
                            password = row[LoginDataSQL.password],
                            ip = row[LoginDataSQL.ip]
                        )
                    }

                for (playerName in cacheKeys) {
                    try {
                        val password = LoginData.password[playerName] ?: continue
                        val ip = LoginData.ipAddress[playerName] ?: "0.0.0.0"
                        val dbRecord = dbRecords[playerName]
                        ...
                        if (dbRecord == null) {
                            ...
                            val diffs = mutableListOf<String>()
                            if (dbRecord.password != password) diffs.add("   Password: DIFERENTE")
                            if (dbRecord.ip != ip) diffs.add("   IP: ${dbRecord.ip} -> $ip")

                            if (diffs.isNotEmpty()) {
                                logs.add("🔄 UPDATE LOGIN: $playerName")
                                logs.addAll(diffs)
                        ...
                        logs.add("❌ ERRO em $playerName: ${e.message}")
                        errors++
                    }
                }
            }

            logs.add("RESUMO LOGIN: ➕$inserted 🔄$updated ✓$unchanged ❌$errors")
            logs.add(if (errors == 0) "✅ LOGIN SYNC OK!" else "⚠️ LOGIN SYNC COM ERROS!")

        } catch (e: Exception) {
            logs.add("❌ ERRO CRÍTICO LOGIN: ${e.message}")
            e.printStackTrace()
        }

        saveLog(logs, "sync-logindata")
        logs.forEach { TotalEssentials.getCore().logger.log(it) }
    }
    ...
    private data class LoginDbRecord(
        val password: String,
        val ip: String
    )
