...
object CoreVip {
    ...
    private val permissionInstance: Any? by lazy {
        try {
            Class.forName("net.milkbowl.vault2.permission.Permission")
            TotalEssentials.getPermission() as Permission
        } catch (e: ClassNotFoundException) {
            try {
                Class.forName("net.milkbowl.vault.permission.Permission")
                TotalEssentials.getPermission() as net.milkbowl.vault.permission.Permission
            } catch (e: ClassNotFoundException) {
                null
            }
        }
    ...
    private fun addVip(entity: String, vipKey: String, perm: Any?, execute: Boolean) {
        ...
        if (token != null && roleID != null && token != 0L) Discord.addUserRole(token, roleID)
    }
