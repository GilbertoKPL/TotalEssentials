...
interface ICommand : CommandExecutor {
    ...
    override fun onCommand(s: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (s !is Player && !consoleCanUse) {
            s.sendMessage(github.genesyspl.cardinal.configs.GeneralLang.getInstance().generalOnlyPlayerCommand)
            return true
        }
        ...
        fun errorMessage() {
            s.sendMessage(github.genesyspl.cardinal.configs.GeneralLang.getInstance().generalCommandsUsage)
            for (it in commandUsage) {
                val to = it.split("_")
                if (to.size == 1) {
                    s.sendMessage(
                        github.genesyspl.cardinal.configs.GeneralLang.getInstance().generalCommandsUsageList.replace(
                            "%command%",
                            it
                        )
                    )
        ...
        try {
            ...
            if (timeCoolDown != null && s is Player && !s.hasPermission("cardinal.bypass.waitcommand")) {
                ...
                if (time != 0L && System.currentTimeMillis() < time) {
                    s.sendMessage(
                        github.genesyspl.cardinal.configs.GeneralLang.getInstance().generalCooldownMoreTime.replace(
                            "%time%",
