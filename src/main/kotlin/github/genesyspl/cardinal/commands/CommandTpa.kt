...
class CommandTpa : ICommand {
    ...
    override fun kCommand(s: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (args[0].lowercase() == s.name.lowercase()) {
            s.sendMessage(github.genesyspl.cardinal.configs.GeneralLang.getInstance().tpaSameName)
            return false
        }
        ...
        if (DataManager.getInstance().tpaHash.contains(s) || DataManager.getInstance().tpaHash.containsValue(p)) {
            s.sendMessage(github.genesyspl.cardinal.configs.GeneralLang.getInstance().tpaAlreadySend)
            return false
        }

        //check if player has telepot request
        if (DataManager.getInstance().tpaHash.contains(p)) {
            s.sendMessage(github.genesyspl.cardinal.configs.GeneralLang.getInstance().tpaAlreadyInAccept)
            return false
        }

        val time = github.genesyspl.cardinal.configs.MainConfig.getInstance().tpaTimeToAccept

        coolDown(s as Player, p, time)

        s.sendMessage(
            github.genesyspl.cardinal.configs.GeneralLang.getInstance().tpaSendSuccess.replace(
                "%player%",
                p.name
            )
        )
        p.sendMessage(
            github.genesyspl.cardinal.configs.GeneralLang.getInstance().tpaOtherReceived.replace("%player%", s.name)
