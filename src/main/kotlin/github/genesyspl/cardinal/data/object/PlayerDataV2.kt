...
import github.genesyspl.cardinal.tables.PlayerDataSQL.FakeNick
...
data class PlayerDataV2(
    ...
    fun setNick(newNick: String, other: Boolean = false): Boolean {
        if (!other) {
            ...
            if (!github.genesyspl.cardinal.configs.MainConfig.getInstance().nicksCanPlayerHaveSameNick && !player.hasPermission(
                    "cardinal.bypass.nickblockednicks"
                )
            ) {
                if (!exist) {
                    return true
    ...
    fun switchVanish(): Boolean {
        ...
        return if (newValue) {
            ...
            ReflectUtil.getInstance().getPlayers().forEach {
                ...
                if (!it.hasPermission("cardinal.commands.vanish") && !it.hasPermission("cardinal.bypass.vanish")) {
