...
import github.genesyspl.cardinal.manager.InternalBukkitObjectInputStream
import github.genesyspl.cardinal.manager.InternalBukkitObjectOutputStream
...
class ItemUtil {
    ...
    fun pickupKit(p: Player, kit: String) {
        ...
        if (!p.hasPermission("cardinal.commands.kit.$kit")) {
            p.sendMessage(github.genesyspl.cardinal.configs.GeneralLang.getInstance().generalNotPerm)
            return
        }
        ...
        if (timeAll >= System.currentTimeMillis() && !p.hasPermission("cardinal.bypass.kitcatch")) {
            val remainingTime = timeAll - System.currentTimeMillis()
            p.sendMessage(
                github.genesyspl.cardinal.configs.GeneralLang.getInstance().kitsCatchMessage.replace(
                    "%time%",
        ...
        if (giveKit(
                p,
                kitCache.items,
                github.genesyspl.cardinal.configs.MainConfig.getInstance().kitsEquipArmorInCatch,
                github.genesyspl.cardinal.configs.MainConfig.getInstance().kitsDropItemsInCatch
            )
        ) {
            playerCache.setKitTime(kit, System.currentTimeMillis())
            p.sendMessage(
                github.genesyspl.cardinal.configs.GeneralLang.getInstance().kitsCatchSuccess.replace(
                    "%kit%",
                    kitCache.fakeName
                )
            )
    ...
    fun giveKit(p: Player, items: List<ItemStack>, armorAutoEquip: Boolean, drop: Boolean = false): Boolean {
        ...
        if (drop) {
            ...
            if (inventorySpace >= itemsInternal.size) {
                ...
                p.sendMessage(
                    github.genesyspl.cardinal.configs.GeneralLang.getInstance().kitsCatchNoSpace.replace(
                        "%slots%",
