...
import github.genesyspl.cardinal.manager.ELang
...
class ConfigUtil {
    ...
    private fun internalReloadConfig() {
        ReflectUtil.getInstance().setValuesFromClass(
            github.genesyspl.cardinal.configs.MainConfig::class.java,
            github.genesyspl.cardinal.configs.MainConfig.getInstance(),
            configYaml
        )
    }
    ...
    private fun startFun(lc: String, lang: Boolean) {
        val directoryStream: DirectoryStream<Path>? = Files.newDirectoryStream(
            FileSystems.newFileSystem(
                Paths.get(github.genesyspl.cardinal.Cardinal.instance.javaClass.protectionDomain.codeSource.location.toURI()),
                github.genesyspl.cardinal.Cardinal.instance.javaClass.classLoader
            ).getPath("/$lc/")
        )
        ...
        if (lang) {
            try {
                if (bol) {
                    ...
                    github.genesyspl.cardinal.configs.MainConfig.getInstance().generalSelectedLang = enum
                ...
                ReflectUtil.getInstance()
                    .setValuesOfClass(
                        github.genesyspl.cardinal.configs.GeneralLang::class.java,
                        github.genesyspl.cardinal.configs.GeneralLang.getInstance(),
                        langYaml
                    )
    ...
    private fun langHelper(): File {
        var langSelected =
            File(
                PluginUtil.getInstance().langPath,
                "${github.genesyspl.cardinal.configs.MainConfig.getInstance().generalSelectedLang}.yml"
            )
    ...
    private fun initializeYml(source: String, lang: Boolean = false): Boolean {
        ...
        try {
            ...
            val resource = github.genesyspl.cardinal.Cardinal.instance.javaClass.getResourceAsStream(location)
