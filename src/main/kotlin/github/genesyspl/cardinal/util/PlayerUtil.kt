...
class PlayerUtil {
    ...
    fun getPlayerUUID(p: Player): String {
        return if (github.genesyspl.cardinal.Cardinal.instance.server.onlineMode) {
            p.uniqueId.toString()
        } else {
            p.name.lowercase()
        }
            ...
            TaskUtil.getInstance().asyncExecutor {
                ...
                DataManager.getInstance().discordChat!!.sendMessageEmbeds(
                    EmbedBuilder().setDescription(
                        github.genesyspl.cardinal.configs.GeneralLang.getInstance().discordchatDiscordSendLoginMessage.replace(
                            "%player%",
                            p.name
                        )
