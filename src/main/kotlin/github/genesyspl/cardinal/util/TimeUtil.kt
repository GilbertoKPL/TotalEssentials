...
class TimeUtil {
    ...
    fun convertMillisToString(time: Long, short: Boolean): String {
        ...
        val uniDays = if (days < 2) {
            github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeDay
        } else github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeDays
        helper(
            days,
            "$days ${github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeDayShort}",
            "$days $uniDays"
        )
        val uniHours = if (hours < 2) {
            github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeHour
        } else github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeHours
        helper(
            hours,
            "$hours ${github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeHourShort}",
            "${hours % 24} $uniHours"
        )
        val uniMinutes = if (minutes < 2) {
            github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeMinute
        } else github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeMinutes
        helper(
            minutes,
            "$minutes ${github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeMinuteShort}",
            "${minutes % 60} $uniMinutes"
        )
        val uniSeconds = if (seconds < 2) {
            github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeSecond
        } else github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeSeconds
        helper(
            seconds,
            "$seconds ${github.genesyspl.cardinal.configs.GeneralLang.getInstance().timeSecondShort}",
            "${seconds % 60} $uniSeconds"
        )
        var toReturn = ""
        var quaint = 0
