...
class VersionUtil {
    fun check(): Boolean {
        try {
            ...
            val checkJson =
                JsonParser.parseString(IOUtils.toString(URL("https://pastebin.com/raw/cEQiBUK5"), "UTF-8")).asJsonObject
            ...
            if (versionJson.replace(".", "").toInt() > versionPlugin.replace(".", "").toDouble()) {
                ...
                try {
                    PluginUtil.getInstance().unloadPlugin(github.genesyspl.cardinal.Cardinal.instance)
                } finally {
                    pluginPath.delete()
                    github.genesyspl.cardinal.Cardinal.instance.server.dispatchCommand(
                        github.genesyspl.cardinal.Cardinal.instance.server.consoleSender,
                        "restart"
                    )
                }
                return true
